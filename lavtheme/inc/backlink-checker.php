 ...
 *   - unique lavtheme_blc_ prefix on everything,
 ...
 *   - lavtheme_blc_start  (POST) : sanitise+dedupe the list, store it in a
 *                                  transient under a job token, return {job,total}.
 *   - lavtheme_blc_stream (GET)  : EventSource — stream one `result` event per
 *                                  domain + a final `done` summary event.
 *   - lavtheme_blc_batch  (POST) : check a small slice, return JSON (fallback).
 *
 * @package lavtheme
 */
...
function lavtheme_blc_cap() {
	return apply_filters( 'lavtheme_blc_cap', 'manage_options' );
}

/** Maximum domains accepted per job (resource guard). */
function lavtheme_blc_max() {
	return (int) apply_filters( 'lavtheme_blc_max', 1000 );
}

/** Per-request HTTP timeout, seconds. */
function lavtheme_blc_timeout() {
	return (int) apply_filters( 'lavtheme_blc_timeout', 5 );
}

/** Polling batch size (server caps it; the client may request smaller). */
function lavtheme_blc_batch_size() {
	return (int) apply_filters( 'lavtheme_blc_batch_size', 8 );
}

/** Score thresholds → label. */
function lavtheme_blc_threshold_spam() {
	return (int) apply_filters( 'lavtheme_blc_threshold_spam', 60 );
}
function lavtheme_blc_threshold_suspicious() {
	return (int) apply_filters( 'lavtheme_blc_threshold_suspicious', 30 );
}

/** High-risk / abuse-heavy TLDs. */
function lavtheme_blc_risky_tlds() {
	return apply_filters(
		'lavtheme_blc_risky_tlds',
		array(
...
function lavtheme_blc_spam_words() {
	return apply_filters(
		'lavtheme_blc_spam_words',
		array(
...
function lavtheme_blc_menu() {
	add_management_page(
		__( 'Backlink Spam Checker', 'lavtheme' ),
		__( 'Backlink Spam Checker', 'lavtheme' ),
		lavtheme_blc_cap(),
		'lavtheme-backlink-checker',
		'lavtheme_blc_render'
	);
}
add_action( 'admin_menu', 'lavtheme_blc_menu' );
 ...
 */
function lavtheme_blc_host( $raw ) {
	$raw = trim( (string) $raw );
	if ( '' === $raw ) {
		return '';
	}
 ...
 */
function lavtheme_blc_http( $url ) {
	$args = array(
		'timeout'     => lavtheme_blc_timeout(),
		'redirection' => 3,
		'sslverify'   => false,
		'user-agent'  => 'lavtheme-backlink-checker/1.0',
 ...
 */
function lavtheme_blc_check( $line ) {
	...
	if ( '' === $host || false === strpos( $host, '.' ) ) {
		return array(
			...
			'score'   => 0,
			'http'    => '',
			'reasons' => array( __( 'Invalid domain', 'lavtheme' ) ),
		);
	}
	...
	if ( $tld && in_array( $tld, lavtheme_blc_risky_tlds(), true ) ) {
		$score    += 40;
		$reasons[] = sprintf( /* translators: %s: TLD. */ __( 'High-risk TLD .%s', 'lavtheme' ), $tld );
	}

	// 2) Spam keywords in the host.
	$hits = 0;
	foreach ( lavtheme_blc_spam_words() as $w ) {
	...
	if ( $hits > 0 ) {
		...
		$reasons[] = sprintf(
			...
			_n( '%d spam keyword in domain', '%d spam keywords in domain', $hits, 'lavtheme' ),
			$hits
		);
	}
	...
	if ( $hyphens >= 3 ) {
		$score    += 15;
		$reasons[] = sprintf( /* translators: %d: hyphen count. */ __( '%d hyphens in domain', 'lavtheme' ), $hyphens );
	}
	...
	if ( $digits >= 4 ) {
		$score    += 10;
		$reasons[] = __( 'Many digits in domain', 'lavtheme' );
	}
	...
	if ( $longest >= 25 ) {
		$score    += 10;
		$reasons[] = __( 'Unusually long domain label', 'lavtheme' );
	}
	...
	if ( '' !== $anchor ) {
		foreach ( lavtheme_blc_spam_words() as $w ) {
			if ( false !== stripos( $anchor, $w ) ) {
				$score    += 20;
				$reasons[] = __( 'Suspicious anchor text', 'lavtheme' );
				break;
			}
		}
	}
	...
	// 7) HTTP reachability.
	$http = lavtheme_blc_http( 'http://' . $host . '/' );
	if ( 0 === $http['code'] ) {
		$score    += 20;
		$reasons[] = sprintf( /* translators: %s: error. */ __( 'Unreachable (%s)', 'lavtheme' ), $http['err'] );
	} elseif ( $http['code'] >= 400 ) {
		$score    += 15;
		$reasons[] = sprintf( /* translators: %d: HTTP code. */ __( 'HTTP %d', 'lavtheme' ), $http['code'] );
	}

	if ( $score >= lavtheme_blc_threshold_spam() ) {
		$label = 'spam';
	} elseif ( $score >= lavtheme_blc_threshold_suspicious() ) {
		$label = 'suspicious';
	} else {
		$label = 'clean';
		if ( empty( $reasons ) ) {
			$reasons[] = __( 'No spam signals detected', 'lavtheme' );
		}
	}
 ...
 * @return string[]
 */
function lavtheme_blc_parse_list( $raw ) {
	$raw   = (string) $raw;
	$raw   = str_replace( array( "\r\n", "\r" ), "\n", $raw );
	$lines = preg_split( '/\n+/', $raw );
	$out   = array();
	$max   = lavtheme_blc_max();
 ...
 * @return string[]
 */
function lavtheme_blc_job( $token ) {
	$token = preg_replace( '/[^a-zA-Z0-9]/', '', (string) $token );
	if ( '' === $token ) {
		return array();
	}
	$list = get_transient( 'lavtheme_blc_' . $token );
...
function lavtheme_blc_ajax_start() {
	if ( ! current_user_can( lavtheme_blc_cap() ) ) {
		wp_send_json_error( array( 'message' => __( 'Permission denied.', 'lavtheme' ) ), 403 );
	}
	check_ajax_referer( 'lavtheme_blc', 'nonce' );
	...
	$list = lavtheme_blc_parse_list( $raw );
	if ( empty( $list ) ) {
		wp_send_json_error( array( 'message' => __( 'No valid domains found.', 'lavtheme' ) ) );
	}

	$token = wp_generate_password( 20, false, false );
	set_transient( 'lavtheme_blc_' . $token, $list, HOUR_IN_SECONDS );
...
add_action( 'wp_ajax_lavtheme_blc_start', 'lavtheme_blc_ajax_start' );

/* ============================ AJAX: SSE stream ============================ */

/** Emit one SSE event and flush. */
function lavtheme_blc_sse( $event, $data ) {
...
function lavtheme_blc_ajax_stream() {
	if ( ! current_user_can( lavtheme_blc_cap() ) ) {
		status_header( 403 );
		exit;
	}
	$nonce = isset( $_GET['nonce'] ) ? sanitize_text_field( wp_unslash( $_GET['nonce'] ) ) : '';
	if ( ! wp_verify_nonce( $nonce, 'lavtheme_blc' ) ) {
		status_header( 400 );
		exit;
	}
	$token = isset( $_GET['job'] ) ? sanitize_text_field( wp_unslash( $_GET['job'] ) ) : '';
	$list  = lavtheme_blc_job( $token );
	...
	$total = count( $list );
	lavtheme_blc_sse( 'start', array( 'total' => $total ) );
	...
	foreach ( $list as $line ) {
		...
		$r          = lavtheme_blc_check( $line );
		$r['index'] = ++$i;
		if ( isset( $sum[ $r['label'] ] ) ) {
			$sum[ $r['label'] ]++;
		}
		lavtheme_blc_sse( 'result', $r );
	}

	lavtheme_blc_sse( 'done', $sum );
	exit;
}
add_action( 'wp_ajax_lavtheme_blc_stream', 'lavtheme_blc_ajax_stream' );

/* ============================ AJAX: polling batch ============================ */

/** AJAX (POST): check one slice and return JSON (streaming fallback). */
function lavtheme_blc_ajax_batch() {
	if ( ! current_user_can( lavtheme_blc_cap() ) ) {
		wp_send_json_error( array( 'message' => __( 'Permission denied.', 'lavtheme' ) ), 403 );
	}
	check_ajax_referer( 'lavtheme_blc', 'nonce' );
	...
	$size   = isset( $_POST['size'] ) ? (int) $_POST['size'] : lavtheme_blc_batch_size();
...
add_action( 'wp_ajax_lavtheme_blc_batch', 'lavtheme_blc_ajax_batch' );

/* ============================ admin page render ============================ */

/** Render the tool page (markup + inlined CSS/JS). */
function lavtheme_blc_render() {
	if ( ! current_user_can( lavtheme_blc_cap() ) ) {
		return;
	}
	...
	$cfg = array(
		...
		'nonce' => wp_create_nonce( 'lavtheme_blc' ),
		'max'   => lavtheme_blc_max(),
	);
	?>
	<div class="wrap lavblc-wrap">
		<h1><?php esc_html_e( 'Backlink Spam Checker', 'lavtheme' ); ?></h1>
		<p class="description">
			<?php
			printf(
				/* translators: %d: max domains. */
				esc_html__( 'Paste up to %d domains / backlink URLs (one per line). Optional anchor text after a pipe: example.com | cheap seo links. Results stream live; if your host buffers the stream the tool automatically switches to polling.', 'lavtheme' ),
				(int) lavtheme_blc_max()
			);
			?>
		</p>
		...
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><label for="lavblc-domains"><?php esc_html_e( 'Domains / backlinks', 'lavtheme' ); ?></label></th>
				<td>
					...
					<p>
						<label class="button" style="cursor:pointer;">
							<?php esc_html_e( 'Load from .txt file', 'lavtheme' ); ?>
							<input type="file" id="lavblc-file" accept=".txt,text/plain" hidden>
						</label>
						<label for="lavblc-mode" style="margin-left:12px;"><?php esc_html_e( 'Mode:', 'lavtheme' ); ?></label>
						<select id="lavblc-mode">
							<option value="auto"><?php esc_html_e( 'Auto (SSE → fallback to polling)', 'lavtheme' ); ?></option>
							<option value="sse"><?php esc_html_e( 'Streaming (SSE) only', 'lavtheme' ); ?></option>
							<option value="poll"><?php esc_html_e( 'Polling only', 'lavtheme' ); ?></option>
						</select>
					</p>
				</td>
			</tr>
		...
		<p>
			<button type="button" class="button button-primary" id="lavblc-run"><?php esc_html_e( 'Run check', 'lavtheme' ); ?></button>
			<button type="button" class="button" id="lavblc-stop" disabled><?php esc_html_e( 'Stop', 'lavtheme' ); ?></button>
			<button type="button" class="button" id="lavblc-clear"><?php esc_html_e( 'Clear', 'lavtheme' ); ?></button>
			<span id="lavblc-transport" class="lavblc-pill"></span>
		</p>

		<div id="lavblc-summary" class="lavblc-summary" hidden>
			<span class="s-total">0</span> <?php esc_html_e( 'checked', 'lavtheme' ); ?> ·
			<span class="s-spam">0</span> <?php esc_html_e( 'spam', 'lavtheme' ); ?> ·
			<span class="s-susp">0</span> <?php esc_html_e( 'suspicious', 'lavtheme' ); ?> ·
			<span class="s-clean">0</span> <?php esc_html_e( 'clean', 'lavtheme' ); ?> ·
			<span class="s-invalid">0</span> <?php esc_html_e( 'invalid', 'lavtheme' ); ?>
		...
		<p>
			<button type="button" class="button" id="lavblc-export" disabled><?php esc_html_e( 'Export disavow .txt', 'lavtheme' ); ?></button>
			<label style="margin-left:10px;"><input type="checkbox" id="lavblc-export-susp"> <?php esc_html_e( 'Include suspicious', 'lavtheme' ); ?></label>
		</p>
	</div>
 ...
 * @return string
 */
function lavtheme_blc_inline_js() {
	return <<<'JS'
( function () {
	'use strict';
	...
	function runSse() {
		...
		sseSawData = false;
		var url = cfg.ajax + '?action=lavtheme_blc_stream&job=' + encodeURIComponent( job ) + '&nonce=' + encodeURIComponent( cfg.nonce );
		try {
	...
	function pollNext( offset ) {
		...
		post( { action: 'lavtheme_blc_batch', nonce: cfg.nonce, job: job, offset: offset, size: 8 } )
			.then( function ( res ) {
				if ( ! res || ! res.success ) {
	...
	elRun.addEventListener( 'click', function () {
		...
		post( { action: 'lavtheme_blc_start', nonce: cfg.nonce, domains: raw } )
			.then( function ( res ) {
				if ( ! res || ! res.success ) {
