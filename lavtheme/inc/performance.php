...
/**
 * Preconnect to the Fontshare origins so the marketplace display fonts (Satoshi
 * / Clash Display, served from api.fontshare.com + cdn.fontshare.com) begin
 * their DNS+TLS handshake during HTML parse instead of after — the homepage's
 * single biggest render-blocking cost (~1s of wasted FCP time without it).
 *
 * @param array  $urls          Resource hints.
 * @param string $relation_type Hint type.
 * @return array
 */
function lavtheme_perf_font_preconnect( $urls, $relation_type ) {
...
add_filter( 'wp_resource_hints', 'lavtheme_perf_font_preconnect', 10, 2 );

/**
 * Load the Fontshare display-font stylesheets without blocking render: serve the
 * <link> as media="print" and flip it to "all" on load. Text paints immediately
 * in the fallback face (the requests already carry display=swap) and re-renders
 * in the brand font once the CSS arrives — removing the render-blocking stall.
 * A <noscript> copy keeps the fonts working with JavaScript disabled.
 *
 * @param string $tag    Stylesheet <link> tag.
 * @param string $handle Style handle.
 * @return string
 */
function lavtheme_perf_async_fonts( $tag, $handle ) {
	if ( is_admin() ) {
		return $tag;
	}
	$async = (array) apply_filters( 'lavtheme_perf_async_style_handles', array( 'lavzen-fonts', 'lavzen-home-fonts' ) );
...
add_filter( 'style_loader_tag', 'lavtheme_perf_async_fonts', 10, 2 );

/**
 * Emit a tiny inline-SVG favicon so browsers stop requesting the non-existent
 * /favicon.ico (a 404 that Lighthouse flags as a console error under Best
 * Practices). Data-URI = zero extra requests. Skipped when a real Site Icon is
 * configured so core stays in charge.
 */
function lavtheme_perf_favicon() {
...
add_action( 'wp_head', 'lavtheme_perf_favicon', 2 );
