...
function lavtheme_enqueue_assets() {
	...
	// ── Unified chrome (app shell + icon rail + topbar) ───────
	// One file, every page (replaces the per-section sidebar/header CSS, which now
	// hold only a pointer). Tokens resolve from global.root.css regardless of order.
	wp_enqueue_style(
		'lavtheme-chrome',
		LAVTHEME_URI . 'assets/css/chrome.css',
		array( 'lavtheme-main' ),
		lavtheme_asset_ver( 'assets/css/chrome.css' )
	);

	// ── LAVZEN unified Liquid-Glass system ────────────────────
	// Loads AFTER the legacy base so the new design wins the cascade. Token +
	// glass material (lavzen-glass) → page/component layer (lavzen-ui) → fixed
	// photo background (lavzen-bg) → behaviour JS (footer). Satoshi via <link>.
	wp_enqueue_style( 'lavzen-fonts', 'https://api.fontshare.com/v2/css?f[]=satoshi@400,500,700,900&display=swap', array(), null );
	if ( is_readable( LAVTHEME_DIR . 'css/lavzen-glass.css' ) ) {
		wp_enqueue_style( 'lavzen-glass', LAVTHEME_URI . 'css/lavzen-glass.css', array( 'lavtheme-main' ), lavtheme_asset_ver( 'css/lavzen-glass.css' ) );
	}
	if ( is_readable( LAVTHEME_DIR . 'css/lavzen-ui.css' ) ) {
		wp_enqueue_style( 'lavzen-ui', LAVTHEME_URI . 'css/lavzen-ui.css', array( 'lavzen-glass' ), lavtheme_asset_ver( 'css/lavzen-ui.css' ) );
	}
	if ( is_readable( LAVTHEME_DIR . 'css/lavzen-bg.css' ) ) {
		wp_enqueue_style( 'lavzen-bg', LAVTHEME_URI . 'css/lavzen-bg.css', array( 'lavzen-ui' ), lavtheme_asset_ver( 'css/lavzen-bg.css' ) );
	}
	if ( is_readable( LAVTHEME_DIR . 'css/lavzen.js' ) ) {
		wp_enqueue_script( 'lavzen-js', LAVTHEME_URI . 'css/lavzen.js', array(), lavtheme_asset_ver( 'css/lavzen.js' ), true );
	}
