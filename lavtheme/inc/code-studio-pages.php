...
function lavtheme_cs_page_ajax_save() {
	...
	// The Template (design) CSS / JS / Mobile-CSS tabs are pre-filled with the
	// page's REAL global asset (main.css / shop.css / checkout.css …). Storing an
	// unchanged copy would make lavtheme_cs_page_head/_footer inject a duplicate of
	// an already-global file on this page, so when the submitted value still matches
	// the file default we drop the override (the page keeps using the global asset).
	// Compare the RAW submission (what the editor showed), not the sanitised copy,
	// and normalise line endings so an untouched save is reliably detected. Real
	// edits differ and are saved + injected as a per-page layer.
	if ( 'design' === $slug && in_array( $type, array( 'css', 'js', 'mcss' ), true ) ) {
		$lav_def_raw = (string) Lav_CS_Source_Reader::default_value( 'page-' . $id, $type );
		$lav_in_n    = trim( str_replace( "\r\n", "\n", (string) $content ) );
		$lav_def_n   = trim( str_replace( "\r\n", "\n", $lav_def_raw ) );
		if ( $lav_in_n === $lav_def_n ) {
			delete_option( $pfkey );
			delete_option( $pfkey . '_empty' );
			wp_send_json_success( array( 'message' => __( 'Matches the theme default — using the global asset.', 'lavtheme' ) ) );
		}
	}
