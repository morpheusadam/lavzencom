...
function lavtheme_shop_sidebar_html() {
	...
	// Show the top-level departments only (not all ~280 sub-categories) so the
	// filter stays usable. The 10 marketplace departments lead, in their canonical
	// order; any other top-level terms that have products follow.
	$terms = array();
	if ( taxonomy_exists( 'download_category' ) ) {
		...
		if ( ! is_wp_error( $top ) && $top ) {
			...
			$ordered = function_exists( 'lavtheme_home_dept_map' ) ? array_keys( lavtheme_home_dept_map() ) : array();
			...
			// Fallback only: if no marketplace departments exist yet, show populated
			// top-level terms so the filter is never empty.
			if ( empty( $terms ) ) {
					...
					foreach ( $terms as $term ) :
						...
						<label class="cat<?php echo $on ? ' active' : ''; ?>">
							...
							<span class="count"><?php
								// Department parents have 0 direct posts; show the subtree total.
								$lav_cnt = function_exists( 'lavtheme_home_dept_count' ) ? lavtheme_home_dept_count( (int) $term->term_id ) : (int) $term->count;
								echo esc_html( number_format_i18n( $lav_cnt ) );
							?></span>
						</label>
						<?php
						$i++;
