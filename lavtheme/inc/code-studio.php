...
function lavtheme_cs_admin_assets( $hook ) {
	...
	foreach ( array( 'text/css', 'text/javascript', 'application/x-httpd-php', 'application/json' ) as $mime ) {
		$opts = $cm_opts;
		// Inline linting for the data-style modes (real errors flagged in the
		// gutter). PHP/HTML is skipped — it can't be statically linted in-browser
		// and would only produce noise on template markup.
		if ( in_array( $mime, array( 'text/css', 'text/javascript', 'application/json' ), true ) ) {
			$opts['lint']    = true;
			$opts['gutters'] = array( 'CodeMirror-linenumbers', 'CodeMirror-foldgutter', 'CodeMirror-lint-markers' );
		}
	...
	// "Pro" layer: command palette, save shortcut, unsaved-change tracking, status
	// bar, find & replace, editor toolbar. Loads AFTER the core so it can read the
	// live CodeMirror instances; never required for the core editors to work.
	$verppcss = function_exists( 'lavtheme_asset_ver' ) ? lavtheme_asset_ver( 'assets/admin/code-studio-pro.css' ) : LAVTHEME_VERSION;
	wp_enqueue_style( 'lavtheme-cs-pro', LAVTHEME_URI . 'assets/admin/code-studio-pro.css', array( 'lavtheme-cs' ), $verppcss );
	$verppjs = function_exists( 'lavtheme_asset_ver' ) ? lavtheme_asset_ver( 'assets/admin/code-studio-pro.js' ) : LAVTHEME_VERSION;
	wp_enqueue_script( 'lavtheme-cs-pro', LAVTHEME_URI . 'assets/admin/code-studio-pro.js', array( 'jquery', 'lavtheme-cs' ), $verppjs, true );
					...
					<div class="lavcs-panel<?php echo $first ? ' is-active' : ''; ?>" data-section="<?php echo esc_attr( $skey ); ?>">
						...
						foreach ( $fields as $type => $label ) :
							...
							<div class="lavcs-editorwrap<?php echo $ftab ? ' is-active' : ''; ?>" data-type="<?php echo esc_attr( $type ); ?>">
								...
								<?php endif; ?>
								<textarea class="lavcs-editor" data-section="<?php echo esc_attr( $skey ); ?>" data-type="<?php echo esc_attr( $type ); ?>" data-mode="<?php echo esc_attr( lavtheme_cs_mode_for( $type ) ); ?>"><?php
									// Render the code INTO the textarea so it is always visible + editable,
									// even if CodeMirror/JS fails to initialise. The JS keeps it in sync.
									echo esc_textarea( lavtheme_cs_get_value( $skey, $type ) );
								?></textarea>
							</div>
							<?php
							$ftab = false;
