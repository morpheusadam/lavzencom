...
( function ( $ ) {
	...
	$( function () {
		...
		function buildPageUI( p ) {
			...
			// Open on the section that actually carries the context's real code (the
			// "Template" / design section) instead of the empty Global override layer,
			// so the user sees real template/CSS/JS the moment a context is selected —
			// the same way the Front Page opens on a code-bearing section.
			var activeIdx = 0;
			for ( var ai = 0; ai < p.sections.length; ai++ ) {
				if ( p.sections[ ai ].slug === 'design' ) { activeIdx = ai; break; }
			}
			...
			p.sections.forEach( function ( sec, idx ) {
				...
				$li.append( '<button type="button" class="lavcs-pnavitem' + ( idx === activeIdx ? ' is-active' : '' ) + '" data-slug="' + escAttr( sec.slug ) + '"><span class="lavcs-navlabel">' + escHtml( sec.label ) + '</span></button>' );
				if ( sec.deletable ) {
				...
				var $panel = $( '<div class="lavcs-panel' + ( idx === activeIdx ? ' is-active' : '' ) + '" data-section="' + escAttr( sid ) + '" data-slug="' + escAttr( sec.slug ) + '"></div>' );
				...
				types.forEach( function ( t, ti ) {
					...
					// Render the real code straight INTO the textarea (via .val so any
					// markup/newlines are preserved verbatim) instead of leaving it empty
					// for lazy initEditor filling. This guarantees every context tab is
					// visible + editable + saveable even if CodeMirror never initialises —
					// matching the server-rendered Front Page editors.
					var initial = ( LavthemeCS.content[ sid ] && typeof LavthemeCS.content[ sid ][ t ] !== 'undefined' )
						? LavthemeCS.content[ sid ][ t ] : '';
					var $ta = $( '<textarea class="lavcs-editor"></textarea>' )
						.attr( 'data-section', sid )
						.attr( 'data-type', t )
						.attr( 'data-mode', pmode( t ) )
						.val( initial == null ? '' : String( initial ) );
					$w.append( $ta );
					$panel.append( $w );
				} );
