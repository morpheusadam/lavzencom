...
/**
 * Inject section + global CSS in the head (after main.css).
 */
function lavtheme_cs_head_css() {
	...
	foreach ( lavtheme_cs_sections() as $key => $section ) {
		...
		// Built-in section CSS already ships in main.css, so only inject an
		// explicit override. Custom sections are NOT in main.css, so inject
		// their file default when there is no override.
